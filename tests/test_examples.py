...
from examples import run_word_count, run_pmi_graph, run_yandex_maps_graph, run_inverted_index_graph
...
def test_word_count_script(word_file: Path, out_file: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(run_word_count.main, [word_file.as_posix(), out_file.as_posix()])
    ...
    output = ops.Read(out_file.as_posix(), ops.json_parser)()
...
def test_pmi_script(pmi_file: Path, out_file: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(run_pmi_graph.main, [pmi_file.as_posix(), out_file.as_posix()])
    assert result.exit_code == 0
...
def test_tf_idf_script(tf_idf_file: Path, out_file: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(run_inverted_index_graph.main, [tf_idf_file.as_posix(), out_file.as_posix()])
    assert result.exit_code == 0
...
@pytest.fixture(scope='session')
def len_file(tmp_path_factory) -> Path:  # type: ignore
    input_path = tmp_path_factory.mktemp('data') / 'input.txt'
    lengths = [
...
def test_yandex_maps_script(time_file: Path, len_file: Path, out_file: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(run_yandex_maps_graph.main,
                           [time_file.as_posix(), len_file.as_posix(), out_file.as_posix()])
    assert result.exit_code == 0
